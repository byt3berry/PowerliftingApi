#[cfg(not(debug_assertions))] use log::warn;
...
mod tests {
    ...
    use std::path::{Path, PathBuf};
    use std::str::FromStr;
    use types::{Country, Division, Equipment, Federation, Sex, Username, Weight, WeightClass};
    ...
    fn test_from_meet_csv_no_error(#[case] path: &str) {
        ...
        let result: Result<MeetData> = MeetDatabase::from_data_csv(&test_file);
    ...
    fn test_from_directory_error(#[case] path: &str) {
        ...
        let result: Result<MeetDatabase> = MeetDatabase::from_directory(&test_file);
    ...
    fn test_from_directory_missing_file(#[case] path: &str) {
        let test_file: PathBuf = Path::new(TEST_PATH).join(path);
        let expected: MeetDatabase = MeetDatabase(Vec::new());
    ...
    fn test_from_directory_1() {
        ...
        let expected: MeetDatabase = MeetDatabase(vec![
            Meet {
                data: MeetData {
                    federation: Federation::FFForce,
                    country: Country::FRANCE,
